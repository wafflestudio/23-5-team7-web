...
export default function CommentsSection({ eventId }: Props) {
  ...
  const me = useMemo(() => {
    try {
      ...
      if (!raw) return null;
      const parsed = JSON.parse(raw) as { nickname?: string; id?: unknown };
  ...
  const loadFirstPage = async () => {
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : '댓글 목록을 불러오지 못했어요.';
      setError(msg);
      setItems([]);
      setNextCursor(null);
      setHasMore(false);
  ...
  const loadMore = async () => {
    ...
    setError(null);
    try {
      const res = await listComments(eventId, { limit: 20, cursor: nextCursor });
      setItems((prev) => [...prev, ...(res.comments ?? [])]);
      setNextCursor(res.next_cursor ?? null);
      setHasMore(Boolean(res.has_more));
    } catch (e) {
      const msg = e instanceof Error ? e.message : '댓글을 더 불러오지 못했어요.';
      setError(msg);
    } finally {
      setLoading(false);
    }
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      setItems((prev) => prev.map((x) => (x.comment_id === updated.comment_id ? updated : x)));
      cancelEdit();
    } catch (e) {
  ...
  return (
    <section className="card" style={{ marginTop: 12 }}>
      <header className="card-header" style={{ justifyContent: 'space-between' }}>
        ...
        <button className="button" type="button" onClick={loadFirstPage} disabled={loading}>
          새로고침
        </button>
      </header>
      ...
      <div className="card-body">
        ...
        <form onSubmit={onSubmit} style={{ display: 'grid', gap: 8, marginBottom: 12 }}>
          <textarea
            ...
            placeholder={isLoggedIn ? '댓글을 입력하세요 (최대 500자)' : '로그인 후 댓글을 작성할 수 있어요'}
            rows={3}
        ...
        {loading && items.length === 0 ? <p className="page-sub">불러오는 중…</p> : null}
        ...
        {items.length === 0 && !loading ? (
          ...
          <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'grid', gap: 10 }}>
            {items.map((c) => {
              ...
              const canManage = Boolean(isLoggedIn && me?.nickname && me.nickname === c.nickname);
              return (
                <li
                  key={c.comment_id}
                  style={{
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12 }}>
                    <div style={{ minWidth: 0 }}>
                      <div style={{ display: 'flex', gap: 8, alignItems: 'baseline' }}>
                        ...
                        <small style={{ color: '#6b7280' }}>{formatKoreanDateTime(c.created_at)}</small>
                        {c.updated_at ? (
                    ...
                    <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                      {canManage ? (
                        <>
                          ...
                          <button className="button" type="button" onClick={() => remove(c.comment_id)}>
                            삭제
                          </button>
                        </>
                      ) : null}
                  ...
                  {!isEditing ? (
                    <p style={{ marginTop: 8, whiteSpace: 'pre-wrap' }}>{c.content}</p>
                  ) : (
                    <div style={{ display: 'grid', gap: 8, marginTop: 8 }}>
                      ...
                      />
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <small style={{ color: '#6b7280' }}>{editingContent.length}/500</small>
                        <div style={{ display: 'flex', gap: 8 }}>
                          <button className="button" type="button" onClick={cancelEdit} disabled={loading}>
                            취소
                          </button>
                          <button className="button primary" type="button" onClick={saveEdit} disabled={loading}>
                            저장
                          </button>
                        </div>
                      </div>
