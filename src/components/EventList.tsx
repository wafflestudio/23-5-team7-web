...
const EventList = ({ refreshKey }: Props) => {
  ...
  const maybeAutoLoadMore = (source: 'scroll' | 'wheel' | 'touchmove') => {
    ...
    if ((import.meta as any).env?.DEV) {
      console.debug('[EventList] autoload', {
        source,
        distanceToBottom,
  ...
  useEffect(() => {
    if ((import.meta as any).env?.DEV) console.debug('[EventList] mounted');
    ...
    return () => {
      ...
      if ((import.meta as any).env?.DEV) console.debug('[EventList] unmounted');
    };
  ...
  const fetchPage = async (opts: { reset?: boolean } = {}) => {
    ...
    if ((import.meta as any).env?.DEV) {
      console.debug('[EventList] fetchPage', {
    ...
    if (loadingRef.current) {
      if ((import.meta as any).env?.DEV) console.debug('[EventList] skip: already loading');
      return;
    }
    if (!hasMoreRef.current && !opts.reset) {
      if ((import.meta as any).env?.DEV) console.debug('[EventList] skip: no more');
      return;
    }
    ...
    if (!opts.reset && cursorRef.current === null) {
      if ((import.meta as any).env?.DEV) console.debug('[EventList] skip: missing cursor');
      return;
    }
  ...
  loadingRef.current = true;
    ...
    try {
      ...
      const nextHasMore = typeof res.has_more === 'boolean' ? res.has_more : inferredHasMore;
  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(
      (entries) => {
        ...
        if ((import.meta as any).env?.DEV) {
          console.debug('[EventList] intersect', {
            isIntersecting: Boolean(first?.isIntersecting),
            hasMore,
            cursor,
            filter,
  ...
  { root: null, rootMargin: '0px 0px -120px 0px', threshold: 0.9 }
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [hasMore, cursor, items.length, filter]);
  ...
  <div ref={loadMoreRef} style={{ height: 120, marginTop: 24 }} />
      ...
      {loading && items.length > 0 ? <p className="page-sub">더 불러오는 중…</p> : null}
      {!loading && !hasMore ? (
