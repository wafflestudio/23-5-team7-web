  ...
  return (
    <section>
      <header className="event-hero">
        ...
        <div className="event-hero-title" style={{ justifyContent: 'space-between' }}>
      ...
      {loading && bets.length === 0 ? <p className="page-sub">불러오는 중…</p> : null}

      <div className="card" style={{ marginTop: 12 }}>
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          <div style={{ fontWeight: 800 }}>내 베팅</div>
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 8, alignItems: 'center' }}>
            <label className="page-sub" style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              상태
              <select
                value={statusFilter}
          ...
          <table className="rank-table">
            ...
            <tbody>
              {bets.length === 0 ? (
                ...
                bets.map((b) => (
                  <tr key={b.bet_id}>
                    <td style={{ fontWeight: 800 }}>{STATUS_LABEL[b.status] ?? b.status}</td>
                    <td style={{ fontWeight: 700 }}>{b.event_title}</td>
                    <td>{b.option_name}</td>
                    <td style={{ textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>
                      {formatNumber(b.amount)}
                    </td>
                    <td style={{ whiteSpace: 'nowrap' }}>{formatDateTime(b.created_at)}</td>
                  </tr>
                ))
              )}
            </tbody>
