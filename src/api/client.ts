...
export async function request<T>(
  ...
  if (import.meta.env.DEV) {
    ...
    const extra = isFormData
      ...
      : undefined;
    // biome-ignore lint/suspicious/noConsole: dev-only request trace
...
function mapErrorMessage(status: number, body: unknown): string {
  ...
  if (status === 400 && code === 'ERR_001')
    return '필수 요청 필드가 누락되었습니다.';
  if (status === 400 && code === 'ERR_003')
    return 'Authorization 헤더 형식이 잘못되었습니다.';
  // Auth/login: ERR_014 is used for invalid credentials (401)
  if (status === 401 && code === 'ERR_014')
    return '이메일 또는 비밀번호가 올바르지 않습니다.';
  if (status === 401 && code === 'ERR_004') return '로그인이 필요합니다.';
  if (status === 401 && code === 'ERR_005')
  ...
  if (status === 400 && code === 'ERR_002')
    return '입력 형식이 올바르지 않습니다.';
  if (status === 409 && code === 'ERR_008')
    return '옵션 이름이 중복되었습니다. 각각 고유해야 합니다.';
  ...
  if (status === 404 && code === 'ERR_012')
    return '선택한 옵션을 찾을 수 없습니다.';
  if (status === 409 && code === 'ERR_013')
    return '이벤트가 OPEN 상태가 아닙니다. (베팅 불가)';
  if (status === 409 && code === 'ERR_014')
    return '이미 해당 이벤트에 베팅했습니다.';

  // Step 7: Comments
  if (status === 400 && code === 'ERR_048')
    return '댓글 내용은 공백만으로 작성할 수 없습니다.';
  if (status === 404 && code === 'ERR_049') return '댓글을 찾을 수 없습니다.';
  if (status === 403 && code === 'ERR_050')
    return '댓글 작성자만 수정/삭제할 수 있습니다.';
  if (status === 404 && code === 'ERR_037')
    return '댓글 목록 커서가 유효하지 않습니다.';
