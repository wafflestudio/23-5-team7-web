...
function toListEventsResult(res: unknown): ListEventsResult {
  ...
  return {
    events: toEventSummaryArray(res),
    next_cursor: null,
    has_more: false,
  };
}
...
// (removed unused legacy helper listEventsArray)
...
export function createEvent(payload: CreateEventRequest) {
  ...
  return request<unknown>(`/api/events`, {
    ...
    if (
      res &&
      typeof res === 'object' &&
      'event_id' in (res as Record<string, unknown>)
    ) {
